...
const MarginPieChart = ({ avgPrice, avgCost, pieData, pieColors, height = 220 }) => (
  <div className="bg-white rounded-lg border border-light-border p-4 h-full flex flex-col">
    <div className="text-text-color font-semibold text-base">Average Margin</div>
    <div className="flex flex-row items-center flex-1">
      ...
      <div className="flex flex-col justify-center gap-1 flex-1">
        <div className="text-text-color text-sm">Total avg cost: ${avgCost.toFixed(1)}</div>
        <div className="text-text-color text-sm">Total avg price: ${avgPrice.toFixed(1)}</div>
      </div>
    </div>
  </div>
);
